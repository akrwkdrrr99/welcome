...
from tkinter import * # __all__ 같이 서브모듈이 잇기에
from tkinter import filedialog # filedialog 를 명시해야함
...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", filetypes=(("PNG 파일", "*.png"), ("모든 파일", "*.*")), \
        initialdir="C:/Users/Administrator/Desktop/코딩연습/파이썬/") # 최초의 C:/경로로 파일 경로를 보여줌
...
def del_file():
    ...
    for index in reversed(list_file.curselection()): # reverse는 뒤집힌 값을 유지, reversed는 새로운 값만 주는것
...
def merge_image(): 
    ...
    try:
        ...
        image_sizes = [] # [(width1, height1), (width2, height3), ...]
        if img_width > -1:
            # width 값 변경
...
txt_dest_path.pack(side="left", fill="x", expand=True, padx=5, pady=5, ipady=4) # ipady는 높이 변경
